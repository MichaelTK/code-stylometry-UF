...
def fetch_samples(path_samples):
    ...
    for
        ...
        sample['class'] = get_groundtruth(sample_id)
        sample['id'] =
...
def get_groundtruth_old(sample_id):
    class_label = None
...
def get_features(raw_sample):
    ...
    if len(raw_sample) > 0:
        ...
        x = 0
        while x < 114: #the two other syntactical features apart from bigrams
            features[90000+x] = raw_sample[x]
            x = x + 1

        #the below 5 statements are for the layout features
        features[90114] = raw_sample[114]
        features[90115] = raw_sample[115]
        features[90116] = raw_sample[116]
        features[90117] = raw_sample[117]
        features[90118] = raw_sample[118]
        features[90119] = raw_sample[119]

        #the ones below are for lexical features
        features[90120] = raw_sample[120]
        features[90121] = raw_sample[121]
        features[90122] = raw_sample[122]
        features[90123] = raw_sample[123]
        features[90124] = raw_sample[124]
        features[90125] = raw_sample[125]
        features[90126] = raw_sample[126]
        features[90127] = raw_sample[127]
        features[90128] = raw_sample[128]
        features[90129] = raw_sample[129]
        features[90130] = raw_sample[130]
        features[90131] = raw_sample[131]
        features[90132] = raw_sample[132]
        features[90133] = raw_sample[133]
        features[90134] = raw_sample[134]
        features[90135] = raw_sample[135]

        #important keywords features
        #x = 90136
        #while x < (90136+33):
        #    features[x] = raw_sample[x-90000]
        #    x = x + 1

        #wordunigrams and ast bigrams past this point
        for elem in raw_sample[136:]:
            features[elem] = 0

        for elem in raw_sample[136:]:
            features[elem] = features[elem] + 1
...
def fetch_samples_as_profiles(profiles_path):
    ...
    for profile in profiles_filenames:
        ...
        with open(os.path.join(profiles_path, profile)) as data_file:
            ...
            #    for combinations in itertools.product(*all_features):
            #        #print combinations
            #        sample['features']['-'.join(combinations)] = 1

            # Discard profiles without images
            #if not valid_image:
            #    continue
...
def fetch_samples(path_samples):
    ...
    samples_pickle = pandas.read_pickle(path_samples)

    x = 0
    while x < len(samples_pickle['code'])+len(samples_pickle['code'])/2:
        try:
            sample = {}
            sample['features'] = get_features(samples_pickle['code'][x])
            sample['class'] = get_groundtruth(samples_pickle['label'][x])
            sample['id'] = samples_pickle['id'][x]
            sample['filename'] = samples_pickle['filename'][x]
            samples.append(sample)
